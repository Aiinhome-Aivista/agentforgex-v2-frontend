...
export default function FileList({ files, removeFile, tab, userInput = '', removeText, disabled }) {
    ...
    <div className="space-y-2 animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      {userInput.trim() && (
        <div className="flex items-center justify-between px-5 py-3 bg-white/[0.02] rounded-2xl border border-white/5 group hover:border-white/10 transition-all">
          <div className="flex items-center gap-4 flex-1 min-w-0">
            <span className={`text-[10px] font-black px-2 py-0.5 rounded-md shrink-0 ${tab === 'websearch'
