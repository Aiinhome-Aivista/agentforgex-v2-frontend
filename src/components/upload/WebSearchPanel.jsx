...
export default function WebSearchPanel({ userInput, setUserInput, disabled }) {
    ...
    <div className="w-full max-w-3xl mx-auto flex flex-col h-full space-y-4">
      ...
      <div className="flex gap-3">
        ...
        <button
          type="button"
          onClick={runSearch}
          disabled={!query.trim() || busy || disabled}
      ...
      {results.length > 0 && (
        <>
          <div className="flex items-center justify-between text-[11px]
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => { if (!disabled) selectAll(); }}
                disabled={disabled}
                className="hover:text-brand-400 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Select all
              </button>
              <span className="text-white/10">|</span>
              <button
                onClick={() => { if (!disabled) clearSel(); }}
                disabled={disabled}
          ...
          <div className="flex-1 overflow-y-auto max-h-[320px] space-y-2 pr-1
            ...
            {results.map((r, i) => {
              ...
              return (
                <div
                  key={r.url || i}
                  onClick={() => { if (!disabled) toggle(r.url); }}
                  className={
                    'group rounded-xl border px-4 py-3 transition-all flex gap-3 items-start ' +
      ...
      {userInput && userInput.trim() && (
        <div className="rounded-xl border border-white/10 bg-white/[0.02]
          ...
          <div className="flex items-center justify-between mb-1.5">
            ...
            <button
              onClick={() => { if (!disabled) setUserInput(''); }}
              disabled={disabled}
              className="text-white/30 hover:text-red-400 inline-flex items-center gap-1 disabled:opacity-40 disabled:cursor-not-allowed"
              title="Clear input"
            >
              <X size={11} /> Clear
            </button>
