...
export default function WorkspacesPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10 space-y-12">
      ...
      {/* Hero Section */}
      <div className="text-center pt-10 space-y-8 max-w-3xl mx-auto">
        <div className="space-y-4">
          <h1 className="text-5xl font-black text-white tracking-tight leading-tight">
            Manage Your <span className="gradient-text">Workspaces</span>
          </h1>
          <p className="text-white/60 text-lg leading-relaxed">
            Keep track of all your process analysis sessions in one place. Jump back into previous workflows or start a new project to discover more automations.
          </p>
        </div>
        ...
        <div className="flex flex-col items-center gap-2">
          <button
            onClick={() => {
              if (atCap) return;
              localStorage.removeItem("user_mission_vision_context");
              navigate('/home');
            }}
            disabled={atCap}
            className={`px-6 py-3 rounded-xl font-bold tracking-tight shadow-lg transition-all inline-flex items-center gap-2 ${
              atCap 
          ...
          {atCap && (
            <p className="text-xs text-brand-500/80 font-bold uppercase tracking-widest mt-2">
              Workspace limit reached. Delete an older workspace to create a new one.
            </p>
          )}
        </div>
      </div>
                ...
                >
                  ...
                  >
                    ...
                    {w.data_size_mb
                      ...
                      : ""}
                    {" · "}
                    {new Date(w.updated_at || w.created_at).toLocaleDateString(undefined, { timeZone: 'UTC' })}
                  </p>
                </div>
