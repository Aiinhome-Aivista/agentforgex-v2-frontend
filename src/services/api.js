 ...
 */
export const downloadSuggestionCode = async (suggestionId, processKey) => {
  // Read chat history from localStorage if processKey is available
  const payload = {};
  if (processKey) {
    const chatHistoryStr = localStorage.getItem(`agentforgex_chat_${processKey}`);
    if (chatHistoryStr) {
      try {
        payload.chat_history = JSON.parse(chatHistoryStr);
      } catch (e) {
        console.warn("Failed to parse chat history for code generation", e);
      }
    }
  }
  ...
  const blob = await api.post(`/suggestions/${suggestionId}/download-code`, payload, {
    responseType: 'blob',
  })
