...
export default function AIThinkingPhase({
  ...
  // A subset of links carry a glowing pulse that travels between nodes,
  // evoking signals firing across the network.
  ...
  return (
    <div ref={containerRef} className="afx-think w-full flex flex-col items-center justify-center py-12 select-none animate-fade-in">
      <style>{`
        ...
        .afx-think .afx-ring-rev { transform-origin: 50% 50%;
          animation: afxRing 14s linear infinite reverse; }
      `}</style>
      ...
      <div className="relative" style={{ width: size, height: size }}>
        ...
        <svg className="afx-ring-rev absolute inset-0" viewBox="0 0 240 240" width={size} height={size}>
          <circle cx="120" cy="120" r="92" fill="none"
            stroke="rgba(34,211,238,.28)" strokeWidth="1" strokeDasharray="2 16" />
        </svg>
        ...
        <svg className="afx-brain relative" viewBox="0 0 240 200" width={size} height={size}>
          ...
          <g>
            {pulseLinks.map((p, i) => (
              <circle key={`pulse-${i}`} r="2.4" fill="#a7f3d0"
                style={{ filter: 'drop-shadow(0 0 4px rgba(110,231,183,0.9))' }}>
                <animateMotion dur={`${p.dur}s`} begin={p.begin}
                  repeatCount="indefinite" path={p.d} />
                <animate attributeName="opacity"
                  values="0;1;1;0" dur={`${p.dur}s`} begin={p.begin}
